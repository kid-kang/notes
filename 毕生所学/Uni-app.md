...
- block标签是虚拟标签
- 获取屏幕信息 查 uni.getSystemInfo
- 在内联标签里修改样式  :style="{...}"
- :class="i==lock?active:' '"可以用三目运算
...
|    属性    |  类型   | 默认值 | 必填 |                      说明                      |
| :--------: | :-----: | :----: | :--: | :--------------------------------------------: |
| selectable | boolean | false  |  否  |                长按文本是否可选                |
...
- :hover-start-time="2000"   按住2秒改变样式
- :hover-stay-time='2000'     不按了 样式会停留2秒
...
|  plain   | Boolean |  false  | 按钮是否镂空，只有边框 |
| disabled | Boolean |  false  |       禁用的按钮       |
| loading  | Boolean |  false  |     显示加载的圈圈     |
...
+ `uni-app` 支持使用字体图标，使用方式与普通 `web` 项目相同，需要注意以下几点：
  ...
  - 字体文件的引用路径推荐使用以 ~@ 开头的绝对路径。
   ...
   @font-face {
     ...
     步骤：1文件导入，2@import 3修改文件中的路径 4类使用
    ```
  
+ 使用sass：插件里安装， 在uni-sass可以使用全局样式
...
- 在npm查  @escook/request-miniprogram   使用
...
```html
<navigator url="/pages/about/about">跳转到关于页面</navigator>  //有返回按钮
open-type="redirect"  删除当前页面进入下一个页面			//无返回按钮 会触发页面销毁
```
...
1. uni.getUserProfile()   用success返回用户信息res.userInfo
2. 存到Vuex和本地，Vuex先读取本地有无(主意json格式)
3. 用uni.login()获取code码并存到Vuex和本地
4. 发送code和userInfo中的rawData到后端，待接收token
5. 后端发送请求到微信验证(需要传入appId,AppSecrey,code),成功的话会返回session_key和openid
6. 后端用sha1加密(session_key+rawData)生成token，并发送到前端
7. 前端接收到token并存到Vuex和本地（之后需要鉴权的请求带上token，后端就能在数据库中验证你这个token ）